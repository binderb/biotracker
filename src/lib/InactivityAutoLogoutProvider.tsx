...
export function InactivityAutoLogoutProvider({
  timeoutMs = +(process.env.NEXT_PUBLIC_TIME_OUT_MS || 10 * 1000), // Inactivity time
  timeoutCheckMs = +(process.env.NEXT_PUBLIC_TIME_OUT_CHECK_MS || 10 * 1000), // Time out to check
  debug = true,
  ...
  useEffect(() => {
    ...
    // events
    const windowEvents: WindowActivityEvent[] = ['focus', 'click', 'keypress']; // 'mousemove', 'scroll',
